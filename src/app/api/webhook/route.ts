...
export async function POST(request: NextRequest) {
  try {
    ...
    // Sesuaikan 'event' dan 'data.qr' jika struktur payload dari mywa berbeda
    if (body && body.qr) { // Mengasumsikan payload langsung berisi 'qr' seperti di log error Anda
      latestQrCode = body.qr;
      console.log('QR Code updated:', latestQrCode);
    } else if (body && body.event === 'qr' && body.data && body.data.qr) { // Struktur alternatif
